...
plot.SANPosterior <- function(posterior, params=NULL, llout=NULL) {
  ...
  llout.cc <- melt(llout.tab, id.vars=c("index", "ll_cc"),
                   measure.vars=paste0("CC", posterior$cc_days))[, day := {
                     as.integer(sub("^CC([0-9]+)$", "\\1", variable))
                   }]
  
  # Plot total organoid sizes
  p.cc <- ggplot2::ggplot() +
    ggplot2::geom_errorbar(data=posterior$logcis_cc, aes(x=day, ymin=10^(logmean-logsd), ymax=10^(logmean+logsd))) +
    ggplot2::geom_line(data=llout.cc, aes(x=day, y=value, color=ll_cc, group=index)) +
  ...
  # Plot rank-sizes
  p.rs <- lapply(posterior$rs_days, function(rs_day) {
    # Extract per-day cell counts as a table suitable for plotting
    llout.rs <- melt(llout.tab, id.vars=c("index", paste0("ll_rs_", rs_day)),
                     measure.vars=paste0("RS", rs_day, "_", posterior$rs_ranks))[, c("ll_rs", "day", "rank") := {
                       list(eval(as.name(paste0("ll_rs_", rs_day))), rs_day, as.integer(sub(paste0("^RS", rs_day, "_([0-9]+)$"), "\\1", variable)))
                     }]
    
    # Plot
    ggplot() +
      ggplot2::geom_errorbar(data=posterior$logcis_rs[day==rs_day],
                             aes(x=rank, ymin=10^(logmean-logsd), ymax=10^(logmean+logsd))) +
      ggplot2::geom_line(data=llout.rs, aes(x=rank, y=value, color=ll_rs, group=index)) +
