...
test_grid_multistep <- function(rates, stopat, samples_per_day=1) {
  tmax <- max(stopat)
  san <- san_deterministic(s0=1, rates=rates, Tmax=stopat[1], samples_per_day=samples_per_day)
  for(s in stopat[2:length(stopat)])
    san <- san_deterministic(previous=san, rates=rates, Tmax=s, samples_per_day=samples_per_day)
